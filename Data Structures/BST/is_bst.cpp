 ...
 **/
void in_order_util(int i, const vector<Node>& tree, vector<int>& result) {
    if (tree[i].left != -1) in_order_util(tree[i].left, tree, result);
    result.push_back(tree[i].key);
    if (tree[i].right != -1) in_order_util(tree[i].right, tree, result);
}
